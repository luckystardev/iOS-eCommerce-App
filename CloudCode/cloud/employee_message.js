...
/*
 * Get list of messsages for EmployeeMessage page
 * 
 * Expected Input:
 ...
 */
Parse.Cloud.define("getMessagesInEmployeeMessagePageWithFilterParams", function(request, response)
{
	var mainQuery = function(config, request)
	{
		...
		query.limit(config.get(constants.kFMConfigLimitPerEmployeeMessage));
 ...
 *			messages: 				List of messages
 */
Parse.Cloud.define("getLatestMessagesInEmployeeMessagePageWithFilterParams", function(request, response)
{
	var mainQuery = function(config, request)
	{
		var otherUser = new Parse.User();
