...
+ (NSDictionary *)camelCasedResponseFromStripeResponse:(NSDictionary *)jsonDictionary
{
    ...
    for (NSString *key in jsonDictionary) {
        ...
        else if ([key isEqualToString:@"bank_account"])
        {
            // Custom section on response for creating a bank account
            attributeDictionary[key] = [self camelCasedResponseFromStripeResponse:[jsonDictionary valueForKey:key]];
        }
        else
...
// ---------------------------------------------------------------------------------------------------------------------------
#pragma mark - Custom Funcitons for Bank Tokens
+ (void)createTokenWithBank:(FMABank *)bank publishableKey:(NSString *)publishableKey completion:(STPCompletionBlock)handler
{
    [self createTokenWithBank:bank publishableKey:publishableKey operationQueue:[NSOperationQueue mainQueue] completion:handler];
}

+ (void)createTokenWithBank:(FMABank *)bank publishableKey:(NSString *)publishableKey operationQueue:(NSOperationQueue *)queue completion:(STPCompletionBlock)handler
{
    if (bank == nil)
        [NSException raise:@"RequiredParameter" format:@"'card' is required to create a token"];
    ...
    NSMutableURLRequest *request = [[NSMutableURLRequest alloc] initWithURL:url];
    request.HTTPMethod = @"POST";
    request.HTTPBody = [self formEncodedDataFromBank:bank];
    [request setValue:[self JSONStringForObject:[self stripeUserAgentDetails]] forHTTPHeaderField:@"X-Stripe-User-Agent"];
...
+ (NSData *)formEncodedDataFromBank:(FMABank *)bank
{
    NSMutableString *body = [NSMutableString string];
    NSDictionary *attributes = [self requestPropertiesFromBank:bank];
    ...
    for (NSString *key in attributes) {
        ...
        [body appendFormat:@"bank_account[%@]=%@", [self URLEncodedString:key], value];
    }
    
    return [body dataUsingEncoding:NSUTF8StringEncoding];
}

+ (NSDictionary *)requestPropertiesFromBank:(FMABank *)bank
{
    return @{@"country"        : bank.countryCode ? bank.countryCode : [NSNull null],
             @"routing_number" : bank.routingNumber ? bank.routingNumber : [NSNull null],
             @"account_number" : bank.accountNumber ? bank.accountNumber : [NSNull null],
            };
}
